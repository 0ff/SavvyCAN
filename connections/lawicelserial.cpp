...
LawicelSerial::LawicelSerial(QString portName) :
    CANConnection(portName, CANCon::LAWICEL_SERIAL, 3, 4000, true),
    mTimer(this) /*NB: set this as parent of timer to manage it from working thread */
{
    qDebug() << "LawicelSerial()";
    debugOutput("LawicelSerial()");
    ...
    gotValidated = true;
    isAutoRestart = false;
    mNumBuses = 1; //the proper number if C/12 is not implemented
...
void LawicelSerial::connectDevice()
{
    ...
    qDebug() << "open serial port " << getPort();
    ...
    QByteArray output;
    output.append("C\r"); // close old conn
    output.append("S6\r"); // set speed @500kBits
    output.append("Z1\r"); // set enable timestamps
    output.append("O\r");  // open Connection
...
void LawicelSerial::procRXChar(unsigned char c)
{
    CANConStatus stats;
    int oldBuses;
    QByteArray output;
    ...
    case IDLE:
        ...
        if (c == 't' || c == 'T' /*|| c == 'r' || c == 'R' */) {
            qDebug() << "BEGIN BUILD_CAN_FRAME";
            rx_state = BUILD_CAN_FRAME;
            rx_step = 0;
            buildFrame.extended = (c == 'T' || c == 'R');
        } else if (c == 'E') {
            rx_state = ERROR;
            rx_step = 0;
            qDebug() << "BEGIN ERROR";
    ...
    case BUILD_CAN_FRAME:
        ...
        int idLen = buildFrame.extended ? 8 : 3;
        int charsBeforeData = idLen;
        if(rx_step <= idLen - 1) {
            // read ID
            buildFrame.ID += b * pow(0x10, idLen-rx_step-1);
            qDebug() << "  READING ID" << (int)buildFrame.ID;
        } else if(rx_step <= charsBeforeData) {
            buildFrame.len = (b & 0xF);
            qDebug() << "  READING LEN" << (int)buildFrame.len;
            if(buildFrame.len > 8) {
                // this was not a valid frame
                qDebug() << "too long a frame found" << buildFrame.len;
                rx_state = IDLE;
                break;
            }
        } else if (rx_step <= charsBeforeData + buildFrame.len*2) {
            int idx = rx_step - charsBeforeData - 1;
            bool oddChar = idx % 2 != 0;
            if(oddChar) {
                idx = idx -1;
            }
            idx = idx / 2;
            qDebug() << "  READING DATA" << idx << c << rx_step << charsBeforeData << oddChar;

            char val = b & 0xF;
            if(oddChar) {
                val = (buildFrame.data[idx] << 4) | val;
            }
            buildFrame.data[idx] = val;
        } else if (rx_step <= buildFrame.len*2 + charsBeforeData + 4) {
            qDebug() << "  SKIPPING TS" << rx_step;
                // switch(rx_step - buildFrame.len - stepsBefore) {
                //     case 0:
                //         // buildFrame.timestamp = b;
                //         buildFrame.timestamp = (b & 0xF) * 0x10;
                //         break;
                //     case 1:
                //         buildFrame.timestamp += (b & 0xF);
                //         break;
                //     case 2:
                //         buildFrame.timestamp += (b & 0xF) * 0x1000;
                //         break;
                //     case 3:
                //         buildFrame.timestamp += (b & 0xF) * 0x100;
        ...
        // {
        // case 0:
        //     buildFrame.ID = b * 0x100;
        //     break;
        // case 1:
        //     buildFrame.ID += b * 0x10;
        //     break;
        // case 2:
        //     buildFrame.ID += b;
        //     break;
        ...
        // default:
        // default:
            // const int stepsBefore = 4;
            // if (rx_step < buildFrame.len + stepsBefore)
            // {
            //     buildFrame.data[rx_step - stepsBefore] = c;
            //     break;
            // }
            // else if (rx_step >= buildFrame.len + stepsBefore && rx_step < buildFrame.len + stepsBefore + 4)
            // {
            //     switch(rx_step - buildFrame.len - stepsBefore) {
            //         case 0:
            //             // buildFrame.timestamp = b;
...
void LawicelSerial::sendCommValidation()
{
    QByteArray output;
